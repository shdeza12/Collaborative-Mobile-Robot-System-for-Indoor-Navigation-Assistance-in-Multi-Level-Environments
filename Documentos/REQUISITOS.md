...
| **RF-15** | Los dos vehículos y el coordinador se **alcanzan por red** con latencia acotada | Medida de ida y vuelta entre los dos vehículos, con **mediana ≤ 100 ms**, **p95 ≤ 250 ms** y **pérdida 0 %** sobre 600 mensajes | ✅ **verificado el 2026-09-14 sobre los dos vehículos físicos.** `amss-ez9n` (carro A, 192.168.0.102) emisor → `amss-jgm9` (carro B, 192.168.0.100) eco: **600 enviados, 600 recibidos, 0 perdidos**; ida y vuelta **mediana 7,61 ms** (cota 100), **p95 20,76 ms** (cota 250), mín 4,74 / máx 58,61. Veredicto `CUMPLE` del propio medidor, con las cotas leídas del código, no del texto. Registro: [`S23_RF15_carroA_carroB.json`](Evidencia/registros/S23_RF15_carroA_carroB.json). **Margen de 13× en la mediana y 12× en la cola**, así que la red no es el factor limitante de RF-08. *(0) Lo que la sesión destapó y no estaba previsto:* **RF-15 no solo esperaba al segundo vehículo, también estaba cortado por `ufw`** —el carro B no tenía ninguna regla para `192.168.0.0/24` y el carro A solo tenía reglas rancias por IP suelta de una sesión anterior, de modo que la pareja carro↔carro, la única que el requisito nombra, estaba bloqueada **en los dos sentidos**; `ping` pasaba al 100 % porque ufw deja pasar ICMP, y el grafo ROS salía vacío. Abierto **por rango y no por IP**, porque los puertos DDS se calculan del dominio y del índice de participante y no son fijos. *(0b) Segunda pareja, y sirve para refutar una conclusión precipitada del mismo día:* carro B (Jazzy) → portátil (Humble) también **CUMPLE** —600 de 600, mediana **8,48 ms**, p95 **45,64 ms**— ([`S23_RF15_carroB_portatil.txt`](Evidencia/logs/S23_RF15_carroB_portatil.txt); su JSON se quedó en el `/tmp` del vehículo, que se descargó antes de copiarlo, y **no se reconstruye a mano**). Con eso **no hay asimetría de serialización entre distribuciones** para `std_msgs/Header`: los pings cruzan Jazzy→Humble y los pongs Humble→Jazzy, los 600 en cada sentido. Las líneas `sequence size exceeds remaining buffer` que hicieron sospecharla **son ruido de descubrimiento ajeno a RF-15**, y lo demuestran dos controles: con el eco solo y nadie publicando salen **21 errores en 30 s y cero rebotes**, y con `ROS_DOMAIN_ID=77` **desaparecen por completo**. **La conclusión errónea vino de una corrida abortada** —la pareja 1 se cortó con `Ctrl-C` antes de imprimir informe y se contaron 32 errores de fondo como si fueran pongs perdidos—; queda anotada porque la regla que deja es útil: *una corrida sin informe impreso no sostiene ninguna conclusión*. *(1) El requisito no traía ninguna cota:* decía «latencia acotada» y en todo el repositorio no había un número. La que se adopta el **2026-09-11, antes de medir**, sale de RF-08: el estado de cada agente se publica a **2 Hz**, medidos 2,000 Hz, o sea **500 ms de período**; una latencia de un sentido de 500 ms dejaría la imagen del robot desfasada una muestra **por construcción**, así que el techo duro es 1000 ms de ida y vuelta y las cotas dejan factor **10** en la mediana y **4** en la cola. *(2) El medidor existe:* [`medir_latencia_red.py`](../herramientas/medir_latencia_red.py), un fichero suelto que solo usa `rclpy` y `std_msgs` —el segundo carro llegará sin workspace, igual que el primero el 7-sep—. Es **de ida y vuelta porque los tres relojes no están sincronizados**: el emisor resta contra su propio reloj y el de la otra máquina no entra en la cuenta. Banco [`prueba_medir_latencia_red.py`](../herramientas/prueba_medir_latencia_red.py) **27/27**, validado mutando el código: tres mutaciones, tres detecciones —y la primera destapó un hueco real, que el banco no fijaba la dirección del redondeo del percentil—. Probado en bucle sobre el portátil: 150/150 pongs, mediana **0,786 ms**, y **veredicto `MEDIDA_LOCAL` negándose a dar cifra** porque el eco corría en la misma máquina, que es la trampa que haría pasar el requisito sin tocar la red. *(3) El guion de la sesión está escrito:* [`HOJA_CAMPO_SEGUNDO_DEEPRACER.md`](HOJA_CAMPO_SEGUNDO_DEEPRACER.md), con las tres parejas a medir —y la que cuenta es **carro↔carro**, porque el portátil no es un vehículo—, la trampa de `ufw` y la de `deepracer.local` ambiguo con dos carros en la red | S19+ |
| **RF-16** | El **mismo código fuente** se despliega en los dos destinos, simulado y físico (decisión D6) | Compilar el coordinador sin cambios en las dos distribuciones y completar una misión **en cada mundo por separado** | 🟡 **verificado con resultado condicionado** (2026-08-18), y **la primera mitad del criterio queda verificada sobre hardware el 2026-09-22** ([`S24_RF16_compilacion_jazzy_hardware.md`](Evidencia/S24_RF16_compilacion_jazzy_hardware.md)). La medida de agosto comparaba definiciones **sin encender hardware**; esta compila, instala y ejecuta. **Lo primero que se comprobó es lo único que puede falsear el requisito de un golpe:** los **22 ficheros** de fuente de `coordinacion` y `coordinacion_msgs` tienen **el mismo md5** en el repositorio y en los dos vehículos — ni un parche de compatibilidad ni un `if ROS_DISTRO`; «sin cambios» es literal. Los tres destinos no se separan solo por ROS: Humble/Ubuntu 22.04/**Python 3.10.12** frente a Jazzy/Ubuntu 24.04/**Python 3.12.3** en los dos carros. `colcon build` dio **4 de 4 `returncode: 0` y `stderr.log` de 0 bytes** en cada vehículo — ni un aviso de obsolescencia al saltar de Python 3.10 a 3.12, que era el riesgo concreto. Las **cinco baterías** corren en los tres destinos con los quince `SALIDA=0`. **La única cifra que no coincidía se persiguió hasta el fondo:** `prueba_agente.py` da 40 en el portátil y 37 + 1 omitida en los carros, y la diferencia **no es de uno sino de tres**, porque el bloque de `prueba_agente.py:116-129` aporta tres comprobaciones si encuentra `CONTRATO_INTERFACES.md` y una sola omisión si no; en el carro se copiaron los paquetes pero no el repositorio, luego 37 + 3 = 40 y lo que no corre **verifica el documento, no el código**. Y no solo compila: ese mismo binario corrió esa tarde en los dos vehículos durante la compuerta **G-4**. **La segunda mitad —«completar una misión en cada mundo»— sigue abierta y cuelga de G-3**, corte C-1 del 2 de octubre; por eso el requisito **sigue en 🟡**. Nada de esto reabre RF-16b | S22 |
...
**Solo RF-15 exigía los dos vehículos, y con el segundo carro entregado el 2026-09-14 quedó
verificado el mismo día: R11 se cierra y OE2 se queda sin celdas rojas.** Las tres pendientes de
arriba **no** se desbloquean con el segundo vehículo —ninguna lo necesitaba—, así que siguen
tal cual.
...
**Veintisiete de treinta y seis requisitos están verificados** al 2026-09-14, y ya no son solo
...
absorbido por RF-29**: el regreso a la escalera es lo que hace la cancelación, no un requisito
separado. Y de los seis parciales, **los tres de OE2 no
