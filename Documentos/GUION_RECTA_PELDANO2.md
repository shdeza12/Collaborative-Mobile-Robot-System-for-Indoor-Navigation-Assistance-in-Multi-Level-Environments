...
| | |
|---|---|
| **Comando** (en el carro, por SSH) | `ssh deepracer@192.168.0.102 "sudo -n bash -c 'source /opt/ros/jazzy/setup.bash && cd ~deepracer && timeout -s INT 100 ros2 bag record -s mcap -o recta_control_1 /rplidar_ros/scan; chown -R deepracer:deepracer ~deepracer/recta_control_1'"` |
| **El `sudo -n` tampoco es adorno, y se descubrió tarde** | Hasta el **2026-09-24** este comando iba como `deepracer`. **No funciona.** Ese día, una pasada de **198 s** dejó un bag con **un solo mensaje** —el `/tf_static` retenido— y **cero** barridos, aunque el grabador había anunciado `Subscribed to topic`. Es la regla del dueño: `deepracer-core` corre como `root`, sus segmentos `/dev/shm/fastrtps_*` son `-rw-r--r-- root root`, y Fast DDS necesita **escribir** en ellos para cerrar el canal de memoria compartida; un suscriptor no-root se queda mudo **sin error alguno**. El mismo instante, como `root`: **7,708 Hz**. El `chown` final —con `;`, para que corra aunque el `timeout` salga distinto de cero— deja el bag recuperable por `scp` sin privilegios. Detalle en el §2 de [`GUION_NAV2_HARDWARE.md`](GUION_NAV2_HARDWARE.md). |
...
| **Esperado** | `ros2 bag info` en el carro debe dar **más de 500 mensajes**. A los 7–10 Hz medidos, 100 s de reloj son ~700 mensajes y una duración de bag de ~94 s, no de 100. El umbral era «más de 100» y se sube el **24-sep**: con 100 pasaría un bag de 150 mensajes, que está roto pero no lo parece. Lo que hay que separar es la pasada sana de la que perdió el sensor, y entre ~700 y un puñado no hay zona gris. |
| **Si falla** | **Si salen 0 mensajes, o un puñado, la primera sospecha es el dueño, no el sensor** — es el síntoma exacto de haber grabado sin `sudo`. Comprueba con `ssh deepracer@192.168.0.102 "sudo -n bash -c 'source /opt/ros/jazzy/setup.bash && timeout 20 ros2 topic hz /rplidar_ros/scan'"`, que debe dar 7–10 Hz: **el `sudo` y el `timeout 20` son los dos obligatorios**, porque sin privilegios no ve nada y con menos de 20 s el descubrimiento no ha terminado y contesta `does not appear to be published yet` sobre un tópico sano. Si sale un `.mcap` de 0 bytes y sin `metadata.yaml`, es que se cortó a mano: queda un grabador huérfano y se mata con `sudo -n pkill -f 'ros2 [b]ag record'` —al morir, vuelca lo grabado y escribe el `metadata.yaml`, así que **mátalo antes de repetir**, no después—. Los corchetes evitan que el patrón coincida con la línea de órdenes del propio `pkill` y se mate a sí mismo. |
