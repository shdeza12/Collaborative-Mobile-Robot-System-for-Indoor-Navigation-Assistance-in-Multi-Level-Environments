...
| Nodos fantasma en `ros2 node list` | Caché del daemon | Añadir `--no-daemon`. **Pero ver la fila de `node list` más abajo: en el vehículo físico esto no basta** |
...
| **`SALIDA=124` en un `ros2 service call`, o `waiting for service to become available`, o `ros2 node list` vacío, contra el VEHÍCULO FÍSICO** | **Estás corriendo como el usuario equivocado.** `deepracer-core.service` declara `User=root`, así que Fast DDS crea sus segmentos de `/dev/shm` con escritura **solo para el dueño**. Un `ros2` lanzado como `deepracer` **descubre por multicast pero no empareja extremos, y no da ningún error: se queda esperando**. Es la misma causa de los `_NODE_NAME_UNKNOWN_` y de los `Node not found` de `param list`. Diagnosticado el 2026-09-22 tras leerse durante tres semanas como «avería del nodo de servos» | **Para leer** servicios y tópicos basta forzar transporte solo-UDPv4: un perfil con `<useBuiltinTransports>false</useBuiltinTransports>` y `FASTRTPS_DEFAULT_PROFILES_FILE` apuntando a él. **Para ACTUAR no basta: hace falta `root`** — medido con cuatro corridas variando usuario y espera. Ver `Evidencia/S24_sonda_actuacion_amss_ez9n.md` §9.2 |
| **Publicas en `/ctrl_pkg/servo_msg` y el vehículo no se mueve, pero no hay ningún error** | Dos causas, y hacen falta **las dos**: correr como `root` **y** esperar **≥ 5 s** tras crear el publicador antes de mandar la primera orden. Con menos espera el emparejamiento no ha terminado y los mensajes se pierden en silencio | Esperar 6 s tras `create_publisher`. **No fiarse de `get_subscription_count()`**: marcó 1 en cuatro corridas, incluidas las tres en que el carro no se movió |
| **Un tópico mide el doble de su frecuencia nominal** (p. ej. `/robotN/estado` a 4 Hz en vez de 2) | **Hay dos instancias del mismo nodo corriendo.** No produce ningún error, ni en el log ni en el grafo: solo duplica la tasa | `ps -eo pid,args \| grep '[c]oordinacion/agente'` y matar las sobrantes. **Medir `ros2 topic hz` antes de cada corrida de campaña**: esto falsearía métricas sin dejar rastro |
| **`ros2 node list` da un número distinto cada vez** (se han visto 21, 15, 10, 0 y «un nodo habiendo tres» en un día) | La introspección de nodos **no es fiable** en el montaje de dos vehículos, ni siquiera con `--no-daemon` | **No usar `node list` para decidir nada.** Usar `ros2 topic info <topico> --verbose`, que nombra nodo y espacio de nombres de cada extremo. Para atribuir una respuesta a un carro concreto, **parar el `deepracer-core` del otro** y contrastar contra su `calibration.json` en disco |
| **El coordinador muere al arrancar con `PackageNotFoundError: 'deepracer_bringup'`** | En los vehículos esa carpeta se copió **sin `package.xml`**, así que colcon la ignora y el respaldo por `get_package_share_directory` falla | Lanzarlo **siempre** con la ruta explícita: `--ros-args -p ruta_puntos:=<ruta al puntos_interes.yaml>`. En hardware esto no es un apaño, es el procedimiento |
| **Un comando parece «no dar datos» y en realidad ni se ejecutó** | Un `--flag` inválido aborta el comando; su mensaje de uso, filtrado con `grep`, se lee igual que «no hay datos». Pasó con `ros2 topic hz --no-daemon`, que **no admite** esa opción | Redirigir a fichero y mirar el **código de salida real**: `cmd > /tmp/o.txt 2>&1; echo $?; cat /tmp/o.txt`. Nunca leer `$?` detrás de una tubería — devuelve el del último comando |
| **`ssh … pkill -f <patron>` devuelve 255 y no mata nada** | `pkill -f` **se mata a sí misma**: el patrón coincide con la línea de órdenes del propio comando remoto. Misma trampa que `pgrep -f` | Partir el patrón con corchetes: `pkill -f 'coordinacion/[a]gente'` |
