...
**Existe desde el 2026-08-24**, compilado en Humble y verificado con un round-trip real por DDS —`coordinacion_msgs/test/prueba_round_trip.py`, **19 de 19 al 2026-09-07**, con los 31 puntos reales del catálogo; la cifra decía «18 de 18» y se había quedado en el 2026-08-24, cuando el catálogo tenía 16 puntos y no existía `RECIBIDA=6`—, no solo con que compile: el paquete tiene que correr también en la tarjeta Jazzy del carro, y generar los headers no prueba que `rmw` transporte igual en las dos distribuciones. **En Jazzy también, desde el 2026-09-07**: compiló en 1 min 31 s sin `stderr` sobre la tarjeta del carro y dio las mismas **19 de 19** con los mismos 31 puntos ([`S22_jazzy_round_trip.txt`](Evidencia/logs/S22_jazzy_round_trip.txt)). Deja de ser una suposición **solo para este paquete**: R8 sigue abierto para `coordinacion`, `deepracer_bringup` y el plugin en C++, que nunca se han construido allí. Y la prueba publica y escucha **dentro de un mismo proceso**, así que no dice nada sobre si las dos máquinas se ven entre sí. Las constantes (`LIBRE=0`…, `INACTIVA=0`…) se declaran en los `.msg` para que ningún nodo escriba el número suelto.
